...
public class FileCheckTask {
	...
	public void checkFile() {
		...
		List<Path> fileListPaths = oldList.stream()
										  .map(dto -> Paths.get("c:\\users\\user\\Desktop\\class\\upload", dto.getUploadPath(), dto.getUuid() + "_" + dto.getFileName()))
										  .collect(Collectors.toList());
		
		// 썸네일 이미지 경로 추출
		oldList.stream()
			   .filter(dto -> dto.isFileType() == true)
		  	   .map(dto -> Paths.get("c:\\users\\user\\Desktop\\class\\upload", dto.getUploadPath(), "s_" + dto.getUuid() + "_" + dto.getFileName()))
		       .forEach(p -> fileListPaths.add(p));
		
		// 폴더 목록과 데이터베이스 파일 목록을 비교
		// 어제날짜 폴더 구하기
		File targetDir = Paths.get("c:\\users\\user\\Desktop\\class\\upload", getYesterDay()).toFile();
